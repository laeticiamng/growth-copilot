...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Parse & analyze - improved parsing for <head> section
    ...
    for (const p of pages) {
      ...
      // Improved parsing - check both <head> and metadata from Firecrawl
      const title = p.metadata?.title || html.match(/<title[^>]*>([^<]*)<\/title>/i)?.[1]?.trim();
      const meta = p.metadata?.description || html.match(/<meta[^>]*name=["']description["'][^>]*content=["']([^"']*)["']/i)?.[1]?.trim() 
                || html.match(/<meta[^>]*content=["']([^"']*)["'][^>]*name=["']description["']/i)?.[1]?.trim();
      const h1Match = html.match(/<h1[^>]*>([^<]*)<\/h1>/i) || html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
      const hasH1 = !!h1Match;
      ...
      if (!hasH1) issues.push({ id: `h-${pageUrl}`, type: 'missing_h1', severity: 'high', title: 'H1 manquant', description: pageUrl, affected_urls: [pageUrl], recommendation: 'Ajouter un H1 unique', ice_score: 68, auto_fixable: false });
    ...
    console.log(`[SEO] Done: ${seenUrls.size} unique pages, ${issues.length} issues (${method})`);

    return new Response(JSON.stringify({
      pages_crawled: seenUrls.size,
      pages_total: seenUrls.size,
      issues,
      errors: [],
