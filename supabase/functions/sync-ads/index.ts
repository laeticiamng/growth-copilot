...
serve(async (req) => {
  ...
  try {
    ...
    const integration = await getIntegration(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, workspace_id, "google_ads");
    if (!integration) {
      return new Response(JSON.stringify({ success: false, error: "Google Ads integration not configured." }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    return new Response(JSON.stringify({ success: true, message: "Sync completed in demo mode" }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
