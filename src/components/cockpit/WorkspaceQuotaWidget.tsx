...
export function WorkspaceQuotaWidget() {
  ...
  const quotas: QuotaItem[] = [
    {
      label: "Tokens IA",
      used: Math.round(tokenUsed / 1000),
      limit: Math.round(tokenLimit / 1000),
      unit: "K",
      icon: Cpu,
      warning: tokenPercent >= 70,
      critical: tokenPercent >= 90,
    },
    {
      label: "Crawls (aujourd'hui)",
      used: crawlsUsed,
      limit: crawlsLimit,
      unit: "",
      icon: Database,
      warning: crawlsPercent >= 70,
      critical: crawlsPercent >= 90,
    },
    {
      label: "Exécutions concurrentes",
      used: runsUsed,
      limit: runsLimit,
      unit: "",
      icon: Zap,
      warning: runsPercent >= 70,
      critical: runsPercent >= 90,
    },
  ];
          ...
          return (
            <div key={item.label} className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className={cn(
                  "text-xs",
                  item.critical ? "text-destructive font-medium" : "text-muted-foreground"
                )}>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Progress 
                      ...
                      className={cn(
                        "h-2",
                        item.critical && "[&>div]:bg-destructive",
                        item.warning && !item.critical && "[&>div]:bg-yellow-500"
                      )}
                    />
                  </TooltipTrigger>
                  <TooltipContent>
                    <p className="text-xs">{percent.toFixed(1)}% utilisé</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
